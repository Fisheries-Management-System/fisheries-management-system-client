...
const TaskDistubation = () => {
    ...
    const loadData = async () => {
      try {
        const { data } = await axios.get(
          "https://pure-atoll-99669.herokuapp.com/employee"
        );
        setEmployeData(data);
      } catch (err) {
        console.error(err);
  ...
  const deleteEmployee = async (id) => {
    ...
    if (sure) {
      const url = `https://pure-atoll-99669.herokuapp.com/employee/${id}`;
      await axios.delete(url).then((response) => {
        const { data } = response;
        if (data) {
          const remaingProduct = employeData.filter((item) => item._id !== id);
  ...
  return (
    <div className="w-full min-h-screen px-1 bg-gray-100 my-5 md:mb-10 lg:pt-8">
      ...
      {modalData && (
        <AssignNewTaskModal
          url={`https://pure-atoll-99669.herokuapp.com/task`}
