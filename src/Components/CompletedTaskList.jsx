...
const CompletedTaskList = () => {
  ...
  const { isLoading, error, data } = useQuery("completedtask", () => fetch(url).then((res) => res.json()));
  const errorText = data?.error;
  return (
    <div>
      <div className="mt-10">
        {isLoading && <LoadingData />}
        {errorText && <p className="text-center text-xl font-semibold text-gray-800">{errorText}</p>}
        {data?.length > 0 && (
                    ...
                    <table className="min-w-full table-auto">
                      <thead>
                        ...
                        <tr className="bg-gray-50 border-b border-gray-200 text-xs leading-4 text-gray-500 uppercase tracking-wider">
                          ...
                          <th className="px-6 py-3 text-center font-medium">Employee Name</th>
                          <th className="px-6 py-3  font-medium text-center">Task Status</th>
                      ...
                      <tbody className="bg-white">
                        {data.map((task) => (
                          <tr key={task._id} className={`${task.completed ? "" : "hidden"}`}>
                              ...
                              <div className="flex flex-col items-center justify-center">
                                <div>
                                  <p className="text-left">
                                    {task.completed ? (
                                      <button className="btn btn-disabled text-white btn-xs bg-teal-500 border-teal-500">Completed</button>
                                    ) : (
                                      <button className="btn btn-disabled text-white btn-xs bg-warning border-warning">INcompleted</button>
                                    )}
                                  </p>
                                </div>
                                <div className="ml-4">
                  ...
                  <div className="align-middle inline-block w-full shadow overflow-x-auto sm:rounded-lg border-b border-gray-200">
                    <table className="min-w-full table-auto">
                      <thead>
                        ...
                        <tr className="bg-gray-50 border-b border-gray-200 text-xs leading-4 text-gray-500 uppercase tracking-wider">
                          <th className="px-6 py-3 text-center font-medium">Employee Info</th>
                      ...
                      <tbody className="bg-white">
                        {data.map((completedTask) => (
                          <tr key={completedTask._id} className={`${completedTask.completed ? "" : "hidden"}`}>
                            <td className="px-6 py-4 whitespace-no-wrap border-b border-gray-200">
                              <div className="text-sm leading-5 text-gray-900">
                                <div className="flex flex-col items-center justify-center">
                                  <div>
                                    <h1>Name: {completedTask.employeeName}</h1>
                            ...
                            <td className="px-6 py-4 whitespace-no-wrap text-center border-b border-gray-200 text-sm leading-5 font-medium">
                              <div className="flex justify-center items-center">
                                <label
                                  htmlFor="task-modal"
                                  onClick={() => setModalData(completedTask)}
                                  className="btn btn-success text-white rounded uppercase font-poppins font-medium"
                                >
                                  Details
                                </label>
        ...
        {modalData && <TaskDetailsModal modalData={modalData} setModalData={setModalData} />}
      </div>
    </div>
  );
};
